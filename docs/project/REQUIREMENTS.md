...
**決定**：Control Plane は `orchestrator core` / `worker adapter` / `LiteLLM gateway` / `workspace runtime` / `oss connectors` を分離する。
...
**決定**：初期実装では、Codex / Claude Code / Google Antigravity を「同一ジョブ契約に従う別種ワーカー」として扱い、役割ベースに選定する。

- `Plan` は LiteLLM 経由の推論ワーカー、または plan-only 実行ができるワーカーを優先する。
- `Dev` は repo 編集と自己テストを行える CLI ワーカーを優先する。
- `Acceptance` は Dev と別ワーカー、または別 Run で実行し、少なくとも担当責務を分離する。
