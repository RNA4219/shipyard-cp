...
| CI | `.github/workflows` に `ci.yml`、`codeql.yml`、`release.yml`、`secret-scan.yml` がある。現行の `ci.yml` は Node 24 LTS / pnpm 9 で backend、frontend、performance、E2E、container smoke、repository gateを分離して実行する。releaseは署名・SBOM・provenance付きGHCR公開までを担当し、実デプロイは利用者側の責務である。 | specified |
