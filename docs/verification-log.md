...
## Retry/Failure/Loop Fingerprint Refactoring (2026-03-18)

`control-plane-store.ts` に retry_count, failure_class, loop_fingerprint 統合を実装:

### 実装内容

- `retryTracker` Map: タスク+ステージごとの再試行回数管理
- `generateLoopFingerprint()`: ループ検出用フィンガープリント生成
- `classifyFailureFromResult()`: 失敗結果から FailureClass を分類
- `dispatch()`: retry_count, loop_fingerprint, retry_policy, lease フィールド追加
- `applyResult()`: 失敗時のリトライ判定、Doom Loop 検出、backoff 計算

### 型定義更新

- `FailureClass` 型エクスポート追加 (`types.ts`)
- `NextAction` に `'retry'` 追加
- `ResultApplyResponse` に `retry_scheduled_at` 追加
- `RetryPolicy` を required fields に統一
