...
where pnpm >nul 2>&1
if %errorlevel% neq 0 (
    echo ERROR: pnpm is not installed. Please install Node.js with Corepack enabled.
    exit /b 1
)
...
call pnpm run build
if %errorlevel% neq 0 (
    echo WARNING: Build failed. You may need to run 'pnpm run build' manually.
) else (
...
echo   3. Run 'pnpm test' to verify installation
echo.
echo For Docker services:
echo   - cd infra\docker ^&^& docker compose up -d
echo.
echo For development:
echo   - pnpm run dev     # Start with hot reload
