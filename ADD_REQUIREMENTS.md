...
本書は、既存の [REQUIREMENTS.md](./REQUIREMENTS.md) と [docs/state-machine.md](./docs/state-machine.md) に対する追加要件を定義する。対象は、長時間実行ジョブとサブエージェント増殖を安全に運用するために不足している次の 6 項目である。
...
| サブエージェント spawn 上限制御 | High | 再帰的な増殖と瞬間的な過負荷を防ぐため |
...
- `agent_spawn_attempt_total{scope}`
- `agent_spawn_allowed_total{scope}`
- `agent_spawn_rejected_total{scope, reason_code}`
- `agent_spawn_queued_total{scope}`
- `agent_active_current{scope}`
- `agent_queue_wait_seconds`
- `agent_rate_tokens_current{scope}`

## 6. サブエージェント spawn 上限制御

### 背景

ワーカーがサブエージェントを再帰的に spawn できる構成では、Task 自体の active job 制御だけでは負荷抑制が不十分である。特に、親エージェントが複数の子エージェントを短時間に連鎖生成すると、worker adapter の上限をすり抜けてジョブツリー全体の実行数が急増し、CPU、メモリ、API 利用量、監査ログ量のいずれも急激に増加する。

既存の同時実行制御は Task / branch / environment などの排他と `publish` の冪等性に主眼があるため、本追補では「サブエージェント増殖速度そのもの」を制御対象として追加する。

### 決定

- サブエージェント spawn には、同時実行上限と時間当たり spawn 上限の二段階制御を設ける。
- 制御は JSON 設定で変更可能とし、既定値は次のとおり固定する。
  - `max_concurrent_agents = 300`
  - `spawn_rate_limit.window_seconds = 60`
  - `spawn_rate_limit.max_spawns_per_window = 150`
  - `spawn_rate_limit.burst = 150`
- 上限超過時の挙動は `queue` または `reject` で表現し、既定は両方とも `queue` とする。
- 本制御は WorkerJob 自体の同時実行制御を置き換えるものではなく、サブエージェント木に対する追加ガードとして扱う。

### 適用範囲

- 対象操作: `subagent_spawn`
- 対象スコープ: `job`, `worker`, `global`
- 対象カウント: 実行中サブエージェント数、60 秒窓の spawn 試行数、queue 待機時間

### 必須要件

- Control Plane または worker adapter は、spawn 要求ごとに `agent_tree_limits` を評価すること。
- `count_descendants = true` の場合、子・孫以下を含む全階層の実行中サブエージェントを同時実行上限の判定対象に含めること。
- `include_root_agent = false` の場合、親エージェント自身を `max_concurrent_agents` のカウント対象に含めないこと。
- `spawn_rate_limit.algorithm = token_bucket` の場合、`burst` を超える瞬間的な spawn を許可してはならないこと。
- `spawn_rate_limit.window_seconds` は 60 固定とし、60 秒あたりの spawn 許容量は `max_spawns_per_window` を超えてはならないこと。
- 同時実行上限超過時は `overflow_policy.on_concurrent_limit_exceeded` に従って `queue` または `reject` を行うこと。
- 分間上限超過時は `overflow_policy.on_rate_limit_exceeded` に従って `queue` または `reject` を行うこと。
- `queue` を選択した場合、`max_queue_wait_seconds` を超えて待機した要求は `AGENT_QUEUE_TIMEOUT` 相当の reason code で失敗させること。
- `reject` または `queue timeout` 時には必ず reason code を返し、監査イベントへ `scope`, `job_id`, `worker_id`, `requested_at`, `active_count`, `remaining_tokens` を記録すること。
- `scope = job` の場合は同一 job ツリー内で判定し、`scope = worker` の場合は同一 worker 実体単位、`scope = global` の場合は shipyard-cp 全体単位で判定すること。

### 設定契約

`agent_tree_limits` は JSON で設定可能とし、最低限次の構造を持たなければならない。

```

### バリデーションルール

- `max_concurrent_agents` は 1 以上の整数であること。
- `spawn_rate_limit.window_seconds` は 60 固定であること。
- `spawn_rate_limit.max_spawns_per_window` は 1 以上の整数であること。
- `spawn_rate_limit.burst` は 1 以上の整数であること。
- `scope` は `job`, `worker`, `global` のいずれかであること。
- `overflow_policy.on_concurrent_limit_exceeded` は `queue` または `reject` のいずれかであること。
- `overflow_policy.on_rate_limit_exceeded` は `queue` または `reject` のいずれかであること。
...
- spawn 要求メタデータ:
  ...
  - `active_count`
  - `remaining_tokens`
  - `queue_expires_at` optional
- spawn 判定結果:
  - `decision`: `allow` / `queue` / `reject`
  - `reason_code` optional
  - `queue_wait_seconds` optional
- policy config:
  - `agent_tree_limits.enabled`
  - `agent_tree_limits.scope`
  - `agent_tree_limits.max_concurrent_agents`
  - `agent_tree_limits.spawn_rate_limit.*`
  - `agent_tree_limits.overflow_policy.*`
  - `agent_tree_limits.observability.*`

### 状態遷移ルール

- spawn 制御違反そのものでは Task の大域状態を変更しなくてよい。
- ただし、queue timeout または連続 reject により下位作業が継続不能となった場合は、`blocked_reason = agent_tree_limit_exceeded` または `blocked_reason = agent_queue_timeout` を用いて `blocked` へ遷移できること。
- `queue` 中の spawn 要求は active job とみなしてはならないが、監査上の pending 要求として追跡できること。

### 受け入れ条件

- 実行中サブエージェント数が 299 のとき、新規 spawn 1 件は成功し、`active_count = 300` になること。
- 実行中サブエージェント数が 300 のとき、新規 spawn 1 件は `queue` または `reject` されること。
- 60 秒内の spawn 成功数が 149 のとき、新規 spawn 1 件は成功すること。
- 60 秒内の spawn 成功数が 150 に達した後の新規 spawn は `queue` または `reject` されること。
- queue された要求は 60 秒以内に枠が空けば実行されること。
- queue 待機が 60 秒を超えた要求は `AGENT_QUEUE_TIMEOUT` で失敗すること。
- 子が子を呼ぶケースでも `count_descendants = true` なら全階層を集計対象に含めること。
- メトリクスと監査ログがすべての spawn 試行で出力されること。
