...
const TaskCard = memo(function TaskCard({ task }: TaskCardProps) {
  return (
    <div className="bg-surface-container p-4 rounded-lg border-l-2 border-outline-variant/30 hover:bg-surface-container-high hover:border-primary/50 transition-all group cursor-pointer">
      {/* Header */}
      <div className="flex justify-between items-start mb-3">
        <StateBadge state={task.state} />
        <span className="material-symbols-outlined text-on-surface-variant text-[16px] opacity-0 group-hover:opacity-100 cursor-pointer hover:text-on-surface">
          more_vert
        </span>
      </div>

      {/* Title */}
      <h3 className="text-sm font-semibold mb-2 text-on-surface line-clamp-2">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-outline-variant/10">
        ...
        <button className="text-primary text-[10px] font-bold uppercase tracking-tighter hover:underline">
          View
        </button>
      </div>
    </div>
...
export const KanbanColumn = memo(function KanbanColumn({ title, tasks, color, count }: KanbanColumnProps) {
  ...
  return (
    <section className={`min-w-[320px] flex-1 max-w-[400px] flex flex-col rounded-lg ${config.column}`}>
      {/* Column Header */}
      <div className="flex items-center justify-between mb-4 px-2 pt-2">
        <div className="flex items-center gap-2">
          <span className={`w-1.5 h-1.5 rounded-full ${config.dot} ${config.dotAnimate}`} />
